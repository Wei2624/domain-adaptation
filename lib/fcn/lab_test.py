...
def im_segment_single_frame(sess, net, im, im_depth, meta_data, num_classes):
    ...
    if cfg.NETWORK == 'FCN8VGG':
        ...
        if cfg.TEST.VERTEX_REG:
            labels_2d, probs, vertex_pred = sess.run(
                [net.get_output('label_2d'), net.get_output('prob_normalized'), net.get_output('vertex_pred')],
                feed_dict=feed_dict)
        else:
...
def im_segment(sess, net, im, im_depth, state, weights, points, meta_data, voxelizer, pose_world2live, pose_live2world):
    ...
    if cfg.INPUT == 'RGBD':
        feed_dict = {net.data: data_blob, net.data_p: data_p_blob, net.gt_label_2d: label_blob, net.state: state,
                     net.weights: weights, net.depth: depth_blob, \
                     net.meta_data: meta_data_blob, net.points: points, net.keep_prob: 1.0}
    else:
        feed_dict = {net.data: data_blob, net.gt_label_2d: label_blob, net.state: state, net.weights: weights,
                     net.depth: depth_blob, \
                     net.meta_data: meta_data_blob, net.points: points, net.keep_prob: 1.0}
    ...
    labels_pred_2d, probs, state, weights, points = sess.run([net.get_output('labels_pred_2d'), net.get_output('probs'), \
                                                              net.get_output('output_state'),
                                                              net.get_output('output_weights'),
                                                              net.get_output('output_points')], feed_dict=feed_dict)
...
def vis_segmentations_vertmaps(im, im_depth, im_labels, im_labels_gt, colors, vertmap_gt, vertmap, labels, labels_gt,
                               poses, intrinsic_matrix):
    """Visual debugging of detections."""
    import matplotlib.pyplot as plt
    from mpl_toolkits.mplot3d import Axes3D
    fig = plt.figure()
    ...
    r = np.linalg.norm(vertmap_gt, axis=2)
    # sin of elevation, sin, cos of azimuth
    elevation_sin = np.zeros_like(r)
    index = np.where(r != 0)
    elevation_sin[index[0], index[1]] = np.sin(
        np.pi / 2 - np.arccos(np.divide(vertmap_gt[index[0], index[1], 2], r[index[0], index[1]])))
    azimuth_sin = np.sin(np.arctan2(vertmap_gt[:, :, 1], vertmap_gt[:, :, 0]))
    azimuth_cos = np.cos(np.arctan2(vertmap_gt[:, :, 1], vertmap_gt[:, :, 0]))

    # show gt vertmap
    ax = fig.add_subplot(232)
    plt.imshow(r)
    ax.set_title('vertex distance gt')

    ax = fig.add_subplot(233)
    plt.imshow(elevation_sin)
    ax.set_title('vertex elevation gt')
    ...
    ax.set_title('vertex elevation')

    # ax = fig.add_subplot(247)
    # plt.imshow(vertmap[:,:,2])
    # ax.set_title('vertex azimuth sin')

    # ax = fig.add_subplot(248)
    # plt.imshow(vertmap[:,:,0])
    # ax.set_title('vertex azimuth cos')
    ...
    for i in perm:
        ...
        if cfg.TEST.VERTEX_REG:
            ...
            if cfg.TEST.RANSAC:
                ...
                poses = RANSAC.estimate_pose(im_depth, probs, vertex_pred[0, :, :, :] / cfg.TRAIN.VERTEX_W,
                                             imdb._extents, fx, fy, px, py, depth_factor)
        ...
        if cfg.TEST.VISUALIZE:
            if cfg.TEST.VERTEX_REG:
                ...
                vertmap_gt = _unscale_vertmap(vertmap_gt, imdb._process_label_image(labels_gt), imdb._extents,
                                              imdb.num_classes)
                print 'visualization'
